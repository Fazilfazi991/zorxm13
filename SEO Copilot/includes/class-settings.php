...
if ( ! class_exists( 'SEO_Copilot_Settings' ) ) {
	...
	class SEO_Copilot_Settings {
		...
		public function page_init() {
			...
			// Sections 1 & 2 are owner-only (User ID 1 or custom capability)
			$is_owner = ( get_current_user_id() === 1 || current_user_can( 'seo_copilot_owner' ) );
		...
		public function sanitize( $input ) {
			...
			// Security: non-owners cannot overwrite API credential fields via crafted POST
			if ( get_current_user_id() !== 1 && ! current_user_can( 'seo_copilot_owner' ) ) {
				$protected_keys = [
					'ai_provider', 'claude_api_key', 'claude_model',
					'gemini_api_key', 'gemini_model', 'monthly_limit',
					'gsc_client_id', 'gsc_client_secret', 'gsc_property_url',
					'dataforseo_email', 'dataforseo_password',
					'ga4_client_id', 'ga4_client_secret', 'ga4_property_id',
				];
				foreach ( $protected_keys as $key ) {
					unset( $input[ $key ] );
					// Also preserve the existing value so it is not wiped
